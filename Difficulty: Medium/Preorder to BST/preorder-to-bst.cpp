...
class Solution {
  public:
    Node* f(int preorder[], int& i, int upperBound, int n){
        if (i == n || preorder[i] > upperBound) return nullptr;
        Node* node = newNode(preorder[i]);
        i++;
        node->left = f(preorder, i, node->data, n);
        node->right = f(preorder, i, upperBound, n);
        return node;
    }
    // Function that constructs BST from its preorder traversal.
    Node* Bst(int preorder[], int size) {
        int i = 0;
        return f(preorder, i, INT_MAX, size);
    }
};
